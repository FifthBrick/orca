...
@ContextConfiguration(classes = [EmbeddedRedisConfiguration, JesqueConfiguration,
  ...
  MockTaskConfig, RollingPushStage, RollingPushStage.PushCompleteTask, DownstreamStage])
class RollingPushStageSpec extends Specification {
   ...
   */
  @Autowired @Qualifier("downstream") Task endTask
  ...
  def "rolling push loops until completion"() {
    ...
    config = [
      application: "app",
      name       : "my-pipeline",
      stages     : [[type: RollingPushStage.PIPELINE_CONFIG_TYPE], [type: "downstream"]],
      version    : 2
    ]
